"""
Architecture-Execution Spectrum analytics.

Pure function: takes a list of ``EnrichedJob`` and returns an
``ArchExecAnalytics`` with score distribution, seniority correlations,
per-company averages, a histogram for the dashboard, and the top
postings on each end of the spectrum.

Thresholds are configurable: scores below ``execution_threshold`` are
execution-heavy (AI-vulnerable), scores above
``architecture_threshold`` are architecture-heavy (future-proof),
anything between is balanced.
"""
...
def compute_arch_exec_analytics(
    ...
    # Per-seniority correlation between arch-exec score and skill count.
    # Skipped for very small cohorts where Pearson r is unstable.
    ...
    logger.info(
        "Arch-exec analytics: mean=%.2f, exec-heavy=%d (%.1f%%), "
        "arch-heavy=%d (%.1f%%)",
